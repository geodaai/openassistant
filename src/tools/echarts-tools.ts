...
export function getEchartsTools(ctx: KeplerContext) {
  ...
  const histogramTool = tool({
    ...
    execute: async ({datasetName, variableName, numberOfBins = 7}, {abortSignal}): Promise<HistogramToolOutput> => {
      try {
  ...
  const boxplotTool = tool({
    ...
    execute: async ({datasetName, variableNames}, {abortSignal}): Promise<BoxplotToolOutput> => {
      try {
        abortSignal?.throwIfAborted();
  ...
  const scatterplotTool = tool({
    ...
    execute: async ({datasetName, xVariableName, yVariableName}, {abortSignal}): Promise<ScatterplotToolOutput> => {
      try {
        abortSignal?.throwIfAborted();
  ...
  const bubbleChartTool = tool({
    ...
    execute: async ({datasetName, xVariableName, yVariableName, sizeVariableName}, {abortSignal}): Promise<BubbleChartToolOutput> => {
      try {
        abortSignal?.throwIfAborted();
  ...
  const pcpTool = tool({
    ...
    execute: async ({datasetName, variableNames}, {abortSignal}): Promise<PCPToolOutput> => {
      try {
        abortSignal?.throwIfAborted();
        const rawData: Record<string, number[]> = {};
