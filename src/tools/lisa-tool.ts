...
Returns cluster assignments, significance values, and creates a LISA map layer in kepler.gl.`,
    ...
    execute: async ({datasetName, variableName, significanceThreshold = 0.05, newDatasetName}, {abortSignal}) => {
      try {
  ...
  const globalMoranTool = tool({
    ...
    execute: async ({datasetName, variableName}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        const values = await getValues(datasetName, variableName);
  ...
  const spatialWeightsTool = tool({
    ...
    execute: async ({datasetName, method, k, distanceThreshold}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        return {
          success: true,
  ...
  const regressionTool = tool({
    ...
    execute: async ({datasetName, dependentVariable, independentVariables, regressionType = 'ols'}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        return {
          success: true,
  ...
  const classifyTool = tool({
    ...
    execute: async ({datasetName, variableName, method, numberOfBins}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        const values = await getValues(datasetName, variableName);
