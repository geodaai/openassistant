...
import {getValuesFromDataset, getGeometriesFromDataset, FETCH_TIMEOUT_MS, combineSignals, mapboxRateLimiter, nominatimRateLimiter} from './utils';
...
export function getGeoTools(ctx: KeplerContext) {
  ...
  const spatialJoinTool = tool({
    ...
    execute: async ({leftDatasetName, rightDatasetName, outputDatasetName}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        return {
          success: true,
  ...
  const bufferTool = tool({
    ...
    execute: async ({datasetName, distance, outputDatasetName}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        return {
          success: true,
  ...
  const dissolveTool = tool({
    ...
    execute: async ({datasetName, groupByVariable, outputDatasetName}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        return {
          success: true,
  ...
  const cartogramTool = tool({
    ...
    execute: async ({datasetName, weightVariable, outputDatasetName}, {abortSignal}) => {
      abortSignal?.throwIfAborted();
      return {
        success: true,
  ...
  const standardizeVariableTool = tool({
    ...
    execute: async ({datasetName, variableName, method = 'zscore', outputDatasetName}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
  ...
  const rateTool = tool({
    ...
    execute: async ({datasetName, eventVariable, baseVariable, outputDatasetName}, {abortSignal}) => {
      try {
        abortSignal?.throwIfAborted();
        return {
          success: true,
