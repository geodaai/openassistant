...
- For any SQL query:
  ...
  3. Use __TABLE__ as the table name placeholder in SQL queries. It will be replaced with the actual DuckDB table name at runtime.
...
- For data analysis:
  1. If new dataset has been generated by tools e.g. spatialQuery, getUsCountyTool, getUsZipcodeTool, isochrone, mergeTables, etc.:
  ...
  3. For using road or line dataset in spatial analysis (e.g. local moran, spatial weights, and spatial join):
     ...
     c. if needed, spatial join by buffered road (left) with points (right) using spatialQuery
...
- For spatial filtering using spatialQuery:
  1. Use ST_Within(ST_GeomFromGeoJSON(l.geometry), ST_GeomFromGeoJSON(r.geometry)) to filter points within polygons
  2. Reference the points dataset as __tbl0__ and polygon dataset as __tbl1__
  3. Save the result and use filterDataset to filter where Count > 0
...
export const ASSISTANT_NAME = 'openassistant2';

export const ASSISTANT_DESCRIPTION = 'An AI Assistant for spatial data analysis';

export const ASSISTANT_VERSION = '0.1.0';
